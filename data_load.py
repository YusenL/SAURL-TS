...
def _get_time_features(dt): #将日期时间信息转换为机器学习算法可以直接利用的数字特征。
    return np.stack([
        dt.minute.to_numpy(),
        dt.hour.to_numpy(),
...
def load_forecast_csv(name, univar=False):
    ...
    return data, train_slice, valid_slice, test_slice, scaler, pred_lens, n_covariate_cols #数据，训练的切片，验证的切片，测试的切片，标准化参数，预测长度，日期变量的列数量
