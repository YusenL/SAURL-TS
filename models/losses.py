...
class AdaptiveBYOLLoss(nn.Module):
    def __init__(self, init_alpha=0.25, init_beta=0.25, init_gamma=0.25):
        ...
        self.alpha = nn.Parameter(torch.tensor(float(init_alpha), requires_grad=True))
        self.beta = nn.Parameter(torch.tensor(float(init_beta), requires_grad=True))
...
def guassian_kernel(source, target, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    ...
    for bandwidth_temp in bandwidth_list:
        # 使用 clamp 来限制输入值的范围，以防止 exp 操作生成 NaN

        L2_distance_safe = torch.clamp(L2_distance, min=1e-8)  # 避免非常小的数值
        bandwidth_temp_safe = torch.clamp(bandwidth_temp, min=1e-8)  # 确保带宽不是零或非常小
        kernel_val.append(torch.exp(torch.clamp(-L2_distance_safe / bandwidth_temp_safe, min=-50, max=50)))

        #kernel_val.append(torch.exp(torch.clamp(-L2_distance / bandwidth_temp, min=-50, max=50)))
...
def compute_Euc_loss(axon, axn):
    # 使用欧氏距离度量差异
    difference = axon - axn
    euclidean_distance = torch.norm(difference, p=2, dim=-1)
    loss = -euclidean_distance.mean()  # 最大化差异，因此最小化负的差异
    return loss
